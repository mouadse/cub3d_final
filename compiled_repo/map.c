...
/*   Updated: 2025/06/02 20:40:13 by msennane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"
static bool	process_map_line_content(t_cub3d *game, bool *is_map_flag,
				bool *map_ended_flag, char *current_line_content,
				int file_line_idx_in_map_block, int current_map_grid_index);

static bool	is_map_content_line(char *line);

void	count_map_size(t_config *config, char *map_file_path)
{
	int		total_lines_in_file;
	char	*line_content;
	int		fd;

	fd = open_file(map_file_path);
	...
	config->grid = ft_calloc(sizeof(char *), (total_lines_in_file + 1));
	if (!config->grid)
		handle_error("Error: Memory allocation failed for map grid (ft_calloc).\n");
	config->grid_rows = total_lines_in_file;
}

void	read_and_copy_map_content(int fd)
{
	t_cub3d	*game;
	char	*current_line;
	...
	while (current_line)
	{
		...
		{
			if (process_map_line_content(game, &is_map_content_started,
					&is_map_content_ended, current_line,
					map_lines_processed_count, current_grid_idx))
			{
				current_grid_idx++;
			}
		}
...
static bool	process_map_line_content(t_cub3d *game, bool *is_map_flag,
				...
				int file_line_idx_in_map_block, int current_map_grid_index)
{
	(void)file_line_idx_in_map_block;
	...
	*is_map_flag = true;
	...
	{
		char *trimmed_line = ft_strtrim(current_line_content, "\n");
		if (!trimmed_line)
		{
...
static bool	is_map_content_line(char *line)
{
	...
	if (ft_strncmp(&line[i], "NO", 2) == 0 || ft_strncmp(&line[i], "SO", 2) == 0 ||
		...
		ft_strncmp(&line[i], "F", 1) == 0 || ft_strncmp(&line[i], "C", 1) == 0)
		return (false);
